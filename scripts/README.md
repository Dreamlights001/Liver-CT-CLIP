...
## HCC Regression (Train/Test Entry)

For the local HCC extension, use `ct_lipro_train.py` for training and `run_zero_shot.py` for test:

```bash
# train only
python ct_lipro_train.py --task regression --train-n 4 --epochs 20 --lr 1e-3 --prompt-template arterial_portal

# test only
python run_zero_shot.py --task regression --stage test --train-n 4 --load-model /path/to/regressor.pt --split-file /path/to/split_manifest.json --prompt-template arterial_portal
```

Training writes per-run logs to the same output subdirectory:
- `train_config.json` (full config snapshot)
- `train_lr_log.csv` (learning rate per step)
- `train_epoch_log.csv` (epoch-level losses and metrics)

`run_train.py` remains the original CT-CLIP pretraining script (contrastive image-text training), and is separate from this HCC downstream workflow.
