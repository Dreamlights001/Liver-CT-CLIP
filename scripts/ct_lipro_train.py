...
from run_zero_shot import parse_args as parse_zero_shot_args
from zero_shot import run_regression
...
if __name__ == '__main__':
    is_hcc_regression = False
    for i, token in enumerate(sys.argv):
        if token == "--task" and i + 1 < len(sys.argv) and sys.argv[i + 1] == "regression":
            is_hcc_regression = True
            break
        if token.startswith("--task=") and token.split("=", 1)[1] == "regression":
            is_hcc_regression = True
            break

    if is_hcc_regression:
        # Reuse unified regression parser, but hard-force training stage here.
        args = parse_zero_shot_args()
        args.task = "regression"
        args.stage = "train"
        print("ct_lipro_train.py: running HCC regression training mode (stage=train).")
        run_regression(args)
    else:
        # Original LiPro classification fine-tuning entry.
